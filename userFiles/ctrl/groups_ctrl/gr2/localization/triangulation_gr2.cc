...
void triangulation(CtrlStruct *cvs)
{
	...
	alpha_1_predicted = (rob_pos->theta + M_PI / 2) + limit_angle( M_PI/2 + ((atan((y_beac_1 - rob_pos->y) / (x_beac_1 - rob_pos->x))))) ;
	alpha_2_predicted = (rob_pos->theta + M_PI/2) + limit_angle( M_PI + ((M_PI / 2) - atan((y_beac_2 - rob_pos->y) / abs(x_beac_2 - rob_pos->x) ))) ;
	alpha_3_predicted = rob_pos->x > 0 ? (rob_pos->theta + M_PI / 2) + limit_angle( (3*  M_PI/2) + (atan((y_beac_3 - rob_pos->y) / (x_beac_3 - rob_pos->x)))) :
		(rob_pos->theta + M_PI / 2) + limit_angle(M_PI / 2 - (atan((y_beac_3 - rob_pos->y) / (x_beac_3 - rob_pos->x))));
